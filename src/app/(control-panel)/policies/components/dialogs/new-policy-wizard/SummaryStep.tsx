...
export const SummaryStep = ({
  ...
  const deliveredItems = checklistItems.filter(
  ...
  return (
    <Stack spacing={2}>
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 3 }}>
        ...
        <SummaryFieldList fields={currentClientFields} data={clientData} />
      </Paper>
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 3 }}>
        ...
        <SummaryFieldList fields={currentBranchFields} data={branchData} />
      </Paper>
            ...
            {deliveredItems.map((item) => {
              ...
              return (
                <Typography key={item.key} variant="body2">
                  - {item.label} ({state?.category || "Cliente"})
                  {expirationText}
                </Typography>
              );
            })}
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 3 }}>
        ...
        {generatedDocuments.length > 0 ? (
          ...
          <Typography color="text.secondary">
            No se genero documento mock aun.
          </Typography>
        )}
      </Paper>
    </Stack>
